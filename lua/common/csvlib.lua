...
local function readFileCSV(filename)
  local content = readFile(filename)
  if content == nil then return nil end
  return jsonDecode(string.format("[[%s]]", content:gsub('\n', '],[')):gsub("([a-zA-Z_])%s*,", '%1",'):gsub(",%s*([a-zA-Z_])", ',"%1'):gsub("([a-zA-Z_])%s*%]", '%1"]'):gsub("%[%s*([a-zA-Z_])", '["%1'), filename)
end
