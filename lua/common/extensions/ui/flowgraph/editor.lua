...
local function vehicleScoringFunction(elem, match)
  ...
  local modelKeyScore = matchStringFunction(elem.info.modelKey, match)
  local configNameScore = matchStringFunction(elem.info.configName, match)
  local configKeyScore = matchStringFunction(elem.info.configKey, match)
  elem.score = math.max(math.max(math.max(modelNameScore, modelKeyScore), configNameScore), configKeyScore)
  return elem.score
end
