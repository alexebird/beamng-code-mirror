...
local function onUpdate()
  if startDebugWindow then
    ...
    if im.Button("Refresh") then
      formattedQuests = career_modules_questManager.getUIFormattedQuests()
    end
    im.SameLine()
    if im.Button("Unlock All") then
      career_modules_questManager.d_unlockAllQuest()
      formattedQuests = career_modules_questManager.getUIFormattedQuests()
    end
    im.SameLine()
    if im.Button("Complete All") then
      career_modules_questManager.d_completeAllQuest()
      formattedQuests = career_modules_questManager.getUIFormattedQuests()
    end
    im.SameLine()
    if im.Button("Claim All") then
      career_modules_questManager.d_claimAllQuest()
      formattedQuests = career_modules_questManager.getUIFormattedQuests()
    end
    if im.CollapsingHeader1("Quest List") then
      ...
      for _, quest in ipairs(formattedQuests) do
        ...
        if im.Button("Complete".. " (" .. quest.id .. ")") then
          career_modules_questManager.d_completeQuest(quest.id)
          formattedQuests = career_modules_questManager.getUIFormattedQuests()
        end
        ...
        if im.Button("Claim".. " (" .. quest.id .. ")") then
          career_modules_questManager.d_claimQuest(quest.id)
          formattedQuests = career_modules_questManager.getUIFormattedQuests()
        end
