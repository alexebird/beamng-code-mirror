...
function C:applyOffset(nx, ny, nz, offset, openxrSessionRunning)
...
function C:switchCamera(carVel, carPos, veh, nx, ny, nz, carStopped, openxrSessionRunning)
  ...
  while true do
    ...
    if openxrSessionRunning then self.isFanMode = false end
...
function C:update(data)
  ...
  if mustFindNewOffset then
    ...
    self:applyOffset(nx, ny, nz, findRandomAttachedNodePosition(data.veh), openxrSessionRunning) -- distance to current vehicle center
  end

  -- check if we should switch to a new camera
  if self:shouldSwitchCamera(carPos, carVel, data.dt) then
    self:switchCamera(carVel, carPos, data.veh, nx, ny, nz, carStopped, data.openxrSessionRunning)
  end
