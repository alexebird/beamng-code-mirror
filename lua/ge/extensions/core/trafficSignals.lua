...
local intersections, controllers, signalMetadata, signalsDict, signalObjectsDict = {}, {}, {}, {}, {}
...
local function setupSignalObjects() -- sets and caches the table of TSStatics that are used as signals
  -- TODO: perhaps the intersection dynamic field is not needed
  ...
  if statics then
    for _, v in ipairs(statics) do -- search for static objects with signal controller dynamic data
      ...
      if interName then
        ...
        local phaseKey = 'phase'..tostring(phaseNum or 0)
        signalObjectsDict[interName] = signalObjectsDict[interName] or {}
        signalObjectsDict[interName][phaseKey] = signalObjectsDict[interName][phaseKey] or {}
        table.insert(signalObjectsDict[interName][phaseKey], v:getId())
      end
    end
  end
end

---- Traffic signal classes go here ----
...
function Intersection:addSignalNode(data)
  ...
  new.dir = data.dir or (self.pos - new.pos):normalized()
...
function Intersection:updateLights(idx, lights)
  ...
  if node then
    for _, v in ipairs(node.objIds) do -- actual traffic signal objects
...
function Intersection:getSignalObjects(sigIdx, refresh) -- returns table of object ids that are used as signals
  if refresh or not next(signalObjectsDict) then setupSignalObjects() end
  local objIds = {}
  if signalObjectsDict[self.name] then
    for k, v in pairs(signalObjectsDict[self.name]) do
      if not sigIdx or string.endswith(k, tostring(sigIdx)) then
        objIds = arrayConcat(objIds, v)
      end
    end
  end
  return objIds
...
function Intersection:setAuxiliaryData() -- applies existing map nodes and positions; also validates the intersection
  local mapNodes = map.getMap().nodes
  self.mapNode = getBestNodes(self.pos)
  if not self.mapNode then
    if not self._invalid then
      log('E', logTag, 'Map node could not be set for intersection: '..self.name)
    end
    self._invalid = true
  end

  for i, sNode in ipairs(self.signalNodes) do
    sNode.mapNode, sNode.prevMapNode = getBestNodes(sNode.pos, sNode.dir)
    if sNode.mapNode then
      sNode.dir = (mapNodes[sNode.mapNode].pos - mapNodes[sNode.prevMapNode].pos):normalized()
    else
      if not self._invalid then
        log('E', logTag, 'Map node could not be set for signal node in intersection: '..self.name)
      end
      self._invalid = true
    end

    sNode.objIds = self:getSignalObjects(i)
  end

  if not self.control then
    self.control = controllers[self.controllerName or 0]
    if not self.control then
      log('E', logTag, 'Signal controller does not exist for intersection: '..self.name)
      self._invalid = true
    end
  end
end
...
function Intersection:onSerialize()
  ...
  for _, v in ipairs(self.signalNodes) do
    if self._moved then -- update direction vector if signal positions were moved in the editor
      v.dir = (self.pos - v.pos):normalized()
    end
    local sData = {
      pos = v.pos:toTable(),
      dir = v.dir:toTable(),
      signalIdx = v.signalIdx
    }
    table.insert(data.signalNodes, sData)
  end
  self._moved = nil
  return data
end
      ...
      if inter.controllerName == self.name then
        ...
        for _, sNode in ipairs(inter.signalNodes) do
          if sNode.mapNode and sNode.signalIdx == sigIdx then
            if next(signalsDict) and signalsDict.nodes[sNode.mapNode] then
  ...
  for _, inter in pairs(intersections) do
    if inter.control and inter.signalNodes[1] then
      for i, sNode in ipairs(inter.signalNodes) do
        if sNode.mapNode then
          nodes[sNode.mapNode] = nodes[sNode.mapNode] or {}
...
local function finalizeSignals() -- finishes signal setup and activates main logic
  for _, inter in pairs(intersections) do
    inter:setAuxiliaryData()
  end

  if next(intersections) and next(controllers) then
    resetTimer()
    buildSignalsDict()
    table.clear(signalObjectsDict) -- to optimize memory usage
    loaded = true
  end
end
...
local function setupSignals(data) -- sets up intersections and controllers, and enables the system
  ...
  if data then
    ...
    for _, v in ipairs(data.intersections) do
      ...
      inter.pos = vec3(v.pos)

      for i, sNode in ipairs(inter.signalNodes) do
        sNode.pos = vec3(sNode.pos)
        sNode.dir = (inter.pos - sNode.pos):normalized()
      end
    end
...
local function onNavgraphReloaded()
  if not next(intersections) and not next(controllers) then -- attempts to auto load signals data for map, if none exists
    loadSignals()
  end
end
...
local function onUpdate(dt, dtSim)
  ...
  if timer == -1 then -- triggers if Lua was reloaded
    finalizeSignals()
  end
  ...
  for iName, inter in pairs(intersections) do
    -- distance check and debug drawing; TODO: quadtree
    local validDist = core_camera.getPosition():squaredDistance(inter.pos) <= viewDistSq -- checks if camera is close enough to intersection
    local changed
    ...
    if debugLevel > 0 and inter._visible then -- basic debug signal spheres
      if debugLevel == 2 then
        if inter._invalid then
          debugDrawer:drawText(inter.pos, String(inter.name..' (ERROR)'), ColorF(1, 0, 0, 1))
        else
          debugDrawer:drawText(inter.pos, String(inter.name..' ('..tostring(inter.controllerName)..')'), ColorF(0, 0, 0, 1))
        end
      end
      ...
      if inter.control then
        for _, sNode in ipairs(inter.signalNodes) do
          local sig = inter.control.signals[sNode.signalIdx]
          if sig then
            local basePos = sNode.pos + vecUp * 4
            local md = inter.control:getMetadata(sNode.signalIdx)
            ...
            if debugLevel == 2 then
              debugDrawer:drawSquarePrism(sNode.pos + sNode.dir * 2, sNode.pos, Point2F(0.5, 0), Point2F(0.5, 1), ColorF(0.25, 1, 0.25, 0.4))
...
local function onSerialize()
  ...
  return {intersections = intersectionsData, controllers = controllersData, active = active, loaded = loaded, debugLevel = debugLevel}
end

local function onDeserialized(data)
  active = data.active
  loaded = data.loaded
  debugLevel = data.debugLevel or 0

  for k, v in pairs(data.intersections) do
    intersections[k] = Intersection:new()
    intersections[k]:onDeserialized(v)
  end
  for k, v in pairs(data.controllers) do
    controllers[k] = SignalController:new()
    controllers[k]:onDeserialized(v)
  end
end
