...
function C:createManualPathnode()
  ...
  if self.mouseInfo.hold then
    ...
    if self.mouseInfo.up then
      -- the old "index = selectedIndex" behavior felt unintuitive and was skipping segments or creating unexpected segments for the user
...
function C:selectNavgraphNode()
  ...
  if target then
    ...
    if self.mouseInfo.down then
      ...
      {index = bestId, self = self, target = target},
      function(data) -- undo
