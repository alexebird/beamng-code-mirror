...
local function drawVehBox(d, isHighlighted)
  local pos, fwd, w, l, h = d.pos, d.dir, d.width, d.length, d.height
  local right = fwd:cross(consts.stdUp)
  right:normalize()
  local up = -fwd:cross(right)
  local wHalf, lHalf, hHalf = w * 0.5, l * 0.5, h * 0.5
...
local function addNode(polyLine, pos, isTimeBased)
  local n1, n2, newX, newY, newZ, len = nil, nil, nil, nil, nil, #polyLine
  ...
  extensions.editor_fileDialog.openFile(
    function(data)
      ...
      for k, data in pairs(vehicles) do
        local veh = core_vehicles.spawnNewVehicle(data.jBeam, { pos = spawnPositions[ctr], config = data.config })
        ctr = ctr + 1
      end
...
local function getWeights(poly, fieldRange, epi)
  local n0, FRInv, weights, len = poly[epi], 1.0 / fieldRange, {}, #poly
  weights[epi] = 1.0
  if epi < len then
    local total = 0.0
...
local function highlightLockSectCam(poly, l, u)
  ...
  for j = l, len do
    ...
    for k = 0, 20 do
      local pNew, dummy = evalTCam(j, (k * dt) * 0.05, false)
...
local function tAdjust(poly, d, l, u, off)
  ...
  local dtL, dtU = poly[l].t[0] - poly[lLock].t[0], poly[uLock].t[0] - poly[u].t[0] -- [Case #4]: There are locks on either side, so use tension/compression method.
  local tMid, ratios = d.tMid, d.ratios
...
local function handleTimeEdit(poly, d, l, u, mouseOffset)
  ...
  local nL, nU = poly[l], poly[u]
  local tL, tU = nL.t[0], nU.t[0]                                                   -- The current lower and upper node times.
  local mid = (tL + tU) * 0.5                                                       -- The midpoint time.
  local dt = tU - tL
  local halfDt = dt * 0.5
...
local function drawTrajectories()
  ...
  if toolWinData.isExecuting == false and cwd.isDisplay[0] == true and numNodes > 0 then
    ...
    debugDrawer:drawTextAdvanced(pStart, "Start", colors.textA, true, false, colors.textB)
    debugDrawer:drawTextAdvanced(pEnd, "End", colors.textA, true, false, colors.textB)
    local lower, upper = getBounds(cwd.nodes)
...
local function fitSplineToTraj(tr)
  ...
  for i = 1, numCubics do
    local c = cubics[i]
    local p1, p2 = coords[i], coords[i + 1]
...
local function onEditorGui()
  ...
  if trajectories == nil or numTrajectories < 1 then
    ...
    local wasAnyVehiclePresent = false
    for k, tr in pairs(trajectories) do
      local polyLine = getPolyRef(tr)
      local len = #polyLine
  ...
  if editor.beginWindow(twd.name, "Script AI Editor", im.WindowFlags_NoTitleBar + im.WindowFlags_NoResize) then
    ...
    elseif twd.tStart >= 0.0 and twd.tEnd > 0.0 then
      if twd.isExecuting == false then
        if editor.uiIconImageButton(editor.icons.simobject_camera, im.ImVec2(30, 30), nil, nil, nil, 'execute') then
    ...
    im.SameLine()
    if editor.uiIconImageButton(editor.icons.time_to_leave, im.ImVec2(29, 29), im.ImVec4(1, 1, 1, vehWinButtonAlpha), nil, nil, 'openVehiclesWindow') then
    ...
    im.SameLine()
    if editor.uiIconImageButton(editor.icons.simobject_path_node, im.ImVec2(27, 27), im.ImVec4(1, 1, 1, trajWinButtonAlpha), nil, nil, 'openTrajectoriesWindow') then
    ...
    im.SameLine()
    if editor.uiIconImageButton(editor.icons.folder_open, im.ImVec2(26, 26), nil, nil, nil, 'loadSession') then
      load()
    end
    im.tooltip('Load session')
    if numTrajectories > 0 and numVehicles > 0 then
      im.SameLine()
      if editor.uiIconImageButton(editor.icons.material_save_current, im.ImVec2(24, 24), nil, nil, nil, 'saveSession') then
        save()
      end
  ...
  if vehWinData.isVisible == true and numVehicles > 0 and drawWinData.isDrawIn == false and twd.isExecuting == false then
    ...
    if editor.beginWindow(vwd.name, "Scene Vehicles") then--, im.WindowFlags_NoResize) then
      im.Separator()
      if im.BeginListBox("", im.ImVec2(470, 180), im.WindowFlags_ChildWindow) then
        ...
        for i = numVehicles, 1, -1 do
          ...
          im.SameLine()
          im.NextColumn()
          if editor.uiIconImageButton(editor.icons.delete_forever, im.ImVec2(22, 22), nil, nil, nil, 'removeVehicle') then
          ...
          im.SameLine()
          im.NextColumn()
          if editor.uiIconImageButton(editor.icons.time_to_leave, im.ImVec2(21, 21), nil, nil, nil, 'goToSelectedVehicle') then
          ...
          if vehicleAttachStates[i] ~= "[not attached]" then
            ...
            if editor.uiIconImageButton(editor.icons.simobject_path_node, im.ImVec2(19, 19), btnCol, nil, nil, 'editTrajectoryVehicleWindow') then
              if idx ~= nil then
                assignTrajWin(idx)
              end
            end
  ...
  if tlwd.isVisible == true and numVehicles > 0 and drawWinData.isDrawIn == false and twd.isExecuting == false then
    if editor.beginWindow(tlwd.name, "Scene Trajectories", im.WindowFlags_NoResize) then
      im.Separator()
      if im.BeginListBox("", im.ImVec2(170, 180), im.WindowFlags_ChildWindow) then
        for k, tr in pairs(trajectories) do
          ...
          im.SameLine()
          im.NextColumn()
          if editor.uiIconImageButton(editor.icons.delete_forever, im.ImVec2(22, 22), nil, nil, nil, 'removeTrajectory') then
          ...
          if numTrajectories > 0 and tr ~= nil then
            ...
            if editor.uiIconImageButton(editor.icons.simobject_path_node, im.ImVec2(19, 19), btnCol, nil, nil, 'editTrajectoryTrajectoryWindow') then
              assignTrajWin(k)
            end
            im.tooltip('Open the linked trajectory window.')
          else
      ...
      im.SameLine()
      if editor.uiIconImageButton(editor.icons.folder_open, im.ImVec2(23, 23), nil, nil, nil, 'importTrajectory') then
        import()
      end
  ...
  if numVehicles > 0 and drawWinData.isDrawIn == false and twd.isExecuting == false then
    for i = 1, 3 do
      ...
      if tIdx ~= nil and trajectories[tIdx] ~= nil then
        ...
        -- Display the window.
        if editor.beginWindow(windowName, "Trajectory - " .. tostring(tIdx), im.WindowFlags_NoResize) then
          ...
          if #polyLine > 1 then
            ...
            if editor.uiIconImageButton(editor.icons.control_point, im.ImVec2(27, 27), nil, nil, nil, 'addNode') then
              if tr.isTimeBased == true then
                local oldNodes = polyPtr2ValT(polyLine)
          ...
          im.Dummy(im.ImVec2(gapB, 0))
          im.SameLine()
          if editor.uiIconImageButton(editor.icons.delete_forever, im.ImVec2(27, 27), nil, nil, nil, 'deleteNode') then
            if tr.isTimeBased == true then
              local oldNodes = polyPtr2ValT(polyLine)
              if #polyLine > 2 then
          ...
          im.Dummy(im.ImVec2(gapB, 0))
          im.SameLine()
          if editor.uiIconImageButton(editor.icons.material_save_current, im.ImVec2(23, 23), nil, nil, nil, 'exportTrajectory') then
            export(polyLine, tr)
          end
          ...
          im.Dummy(im.ImVec2(gapB, 0))
          im.SameLine()
          if editor.uiIconImageButton(editor.icons.simobject_path_node, im.ImVec2(24, 24), nil, nil, nil, 'revealTrajectory') then
          ...
          if tr.vehicle == nil or tr.vehicle == "" then
            ...
            im.SameLine()
            if editor.uiIconImageButton(editor.icons.attachment, im.ImVec2(26, 26), nil, nil, nil, 'attachTrajectoryToVehicle') then
            ...
            im.SameLine()
            if editor.uiIconImageButton(editor.icons.content_cut, im.ImVec2(22, 22), nil, nil, nil, 'detachTrajectoryFromVehicle') then
              local oldTraj = ptr2ValTraj(tr)
              tr.vehicle, tr.vid, tr.jBeam = "", nil, nil
              local newTraj = ptr2ValTraj(tr)
              local data = { old = oldTraj, new = newTraj, tIdx = tIdx }
            ...
            im.SameLine()
            if editor.uiIconImageButton(editor.icons.time_to_leave, im.ImVec2(24, 24), nil, nil, nil, 'goToLinkedVehicle') then
              core_camera.setByName(0, "orbit", false)
          ...
          if tr.isUseSpline[0] == true then
            ...
            im.SliderInt("##2", tr.splineSpacing, 1, 20, "Spacing = %.3f")
            im.PopItemWidth()
            im.PopStyleVar()
  ...
  if cwd.isVisible == true and numVehicles > 0 and drawWinData.isDrawIn == false and twd.isExecuting == false then
    ...
    if editor.beginWindow(camWinData.name, "Camera Trajectory", im.WindowFlags_NoResize) then
      ...
      if numNodes < 1 or tNow > nodes[numNodes].t[0] then
        if editor.uiIconImageButton(editor.icons.control_point, im.ImVec2(26, 26), nil, nil, nil, 'addCamNode') then
      ...
      im.NextColumn()
      if numNodes > 0 then
        if editor.uiIconImageButton(editor.icons.delete_forever, im.ImVec2(26, 26), nil, nil, nil, 'removeCamNode') then
          local oldData = serializeCamData()
      ...
      im.SameLine()
      im.NextColumn()
      if editor.uiIconImageButton(editor.icons.simobject_path_node, im.ImVec2(24, 24), nil, nil, nil, 'camTrajReveal') then
  ...
  if dwd.isDrawIn == true and twd.isExecuting == false then
    if editor.beginWindow(dwd.name, "Draw Trajectory", im.WindowFlags_NoResize) then
