...
local laneColours = {                                                                               -- The lane type colours, in the road surface visualisation.
  ...
  concrete = color(145, 117, 103, 255) }
...
local function renderLaneDirectionTriangles(renderData, laneKeys, isGroupMode, camPos)
  ...
  for j = 1, numDivs do
    ...
    for k = 1, numLaneKeys do
      local lane = laneKeys[k]
      local dL = div[lane]
      if dL[8] ~= 'island' and dL[8] ~= 'concrete' then                                   -- Do not draw lane direction info for island/concrete lanes.
...
local function drawRoadMarkups(isGroupMode, isProfileMode, isCreateGroup, gPolygon, isLinkMode, roads, map, link)
