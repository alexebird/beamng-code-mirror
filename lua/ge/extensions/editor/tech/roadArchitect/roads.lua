...
local meshMgr = require('editor/tech/roadArchitect/roadMesh')                                       -- A module for managing procedural road meshes.
...
local function createRoadFromProfile(profile)
  ...
  return {
    ...
    isStartLineDecal = im.BoolPtr(true),                                                            -- Decals: indicates if the road start line (at junction) decals will appear.
    isEndLineDecal = im.BoolPtr(true),                                                              -- Decals: indicates if the road end line (at junction) decals will appear.
...
local function removeAll()
  ...
  local numRoads = #roads
  for i = 1, numRoads do
    meshMgr.tryRemove(roads[i].name)                                                                -- Remove all meshes, if any exist.
...
local function updateCameraPose()
  gView:set(0.0, 20.0, auditionHeight + 10.0)
...
local function unfinalise()
  ...
  for i = 1, numRoads do
    ...
    meshMgr.tryRemove(roadName)                                                                     -- If any mesh was created for this road on finalise, remove it now.
...
local function deserialiseRoad(rSer)
  ...
  return {
    ...
    isConformRoadToTerrain = im.BoolPtr(rSer.isConformRoadToTerrain or false),
    isDisplayRoadSurface = im.BoolPtr(rSer.isDisplayRoadSurface or true),
    isDisplayRoadOutline = im.BoolPtr(rSer.isDisplayRoadOutline or true),
    isDisplayNodeSpheres = im.BoolPtr(rSer.isDisplayNodeSpheres or true),
    isDisplayNodeNumbers = im.BoolPtr(rSer.isDisplayNodeNumbers or false),
    isDisplayRefLine = im.BoolPtr(rSer.isDisplayRefLine or true),
    isAllowTunnels = im.BoolPtr(rSer.isAllowTunnels or false),
    isRefLineDecal = im.BoolPtr(rSer.isRefLineDecal or true),
    isEdgeLineDecal = im.BoolPtr(rSer.isEdgeLineDecal or true),
    isLaneDivsDecal = im.BoolPtr(rSer.isLaneDivsDecal or true),
    isStartLineDecal = im.BoolPtr(rSer.isStartLineDecal or true),
    isEndLineDecal = im.BoolPtr(rSer.isEndLineDecal or true),
    isDisplayLaneInfo = im.BoolPtr(rSer.isDisplayLaneInfo or true),
