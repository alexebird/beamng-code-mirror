...
function C:update(mouseInfo)
  ...
  local change = false
  if self:correctEditMode() then
    if self._beginCooldown and self._beginCooldown > 0 then
...
function C:enableEditing(cooldown)
  cooldown = cooldown or 1
  self:updateTransform()
  editor.selectEditMode(editor.editModes[self.editName])
  self._beginCooldown = cooldown
end
...
function C:resetDragging() -- this method can be manually triggered to clear data
  self.beginDragPos, self.beginDragRot, self.beginDragScl = nil, nil, nil
end

-- helper function for getting if the correct edit mode is active
function C:correctEditMode()
  return (editor.editMode and editor.editMode.editName == self.editName) or false
end
