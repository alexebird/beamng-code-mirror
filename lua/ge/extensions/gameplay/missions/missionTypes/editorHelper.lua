...
local newDataFunctions = {
  ...
  missionId = defaultValueGetNewData,
}
...
local function modelConfigCheckMission(e, mtd, m)
  local issues = {}
  ...
  local config = model.configs[mtd[e.fieldNameConfig]]
  if not config then
...
local function fileCheckMission(e, mtd, m)
...
local function fixedFileCheckMission(e, mtd, m)
  if e.displayOptions.optional then return end
  local foundFile = nil
...
local function missionIdCheckMission(e, mtd, m)
  if e.displayOptions.optional then return end
...
local checkMissionFunctions = {
  modelconfig = modelConfigCheckMission,
  file = fileCheckMission,
  fixedFile = fixedFileCheckMission,
  race = fileCheckMission,
  sites = fileCheckMission,
  missionId = missionIdSetMission,
}
