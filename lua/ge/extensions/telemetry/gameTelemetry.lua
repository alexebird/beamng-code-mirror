...
local activityAggregationKeys = {LevelLoaded = {"LevelName"}, TrafficEnabled = {}, ScenarioRunning = {"File"}, ScenarioEnded = {"File", "Result"},
                      ...
                      VehicleResetted = {"Vehicle"}, UIStateUsed = {"State"}, ControlsUsed = {"Method"}, TrackBuilder = {}, BigMapActive = {}, MissionRunning = {"MissionId", "Author"}, MissionStarEarned = {"MissionId", "Star"}}
...
local function makeReport(gameSessionReport)
  ...
  --jsonWriteFile("Report.json", data, true)
...
local function onActivateBigMapCallback()
  startTracking({Name = "BigMapActive"})
  writeReportToFileRequested = true
end

local function onDeactivateBigMapCallback()
  stopTracking({Name = "BigMapActive"})
  writeReportToFileRequested = true
end

local function onAnyMissionChanged(change, mission, userSetting)
  if change == "started" then
    startTracking({Name = "MissionRunning", MissionId = mission.id, Author = mission.author, UserSettings = userSettings or {}})
  elseif change == "stopped" then
    stopTracking({Name = "MissionRunning", MissionId = mission.id, Author = mission.author})
  end
  writeReportToFileRequested = true
end
...
local function onMissionAttemptAggregated(attempt, mission)
  local att = {}
  for key, unlocked in pairs(attempt.unlockedStars) do
    if unlocked then
      addSingleActivity({Name = "MissionStarEarned", Star = key, MissionId = mission.id})
      writeReportToFileRequested = true
    end
  end
end
