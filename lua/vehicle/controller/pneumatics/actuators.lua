...
local function updateGFX(dt)
  -- with no air tank installed, there is infinite pressure available
  local relTankPressurePsi = airTank and ((airTank.currentPressure - powertrain.currentEnvPressure) * pascalToPsi) or math.huge

  for k, g in pairs(beamGroups) do
    ...
    for _, v in pairs(g.beams) do
      ...
      if k == "frontDoors" then
        --streams.drawGraph(k .. "_current", { value = v.currentPressure })
        --streams.drawGraph(k .. "_target", { value = v.targetPressure })
      end

      -- TODO: should operate on energy, not on pressure
      local prevPressure = v.currentPressure

      if v.currentPressure ~= v.targetPressure then
        isFinishedChangingPressure = false
        local clampedTargetPressure

        if v.currentPressure >= v.targetPressure then
          clampedTargetPressure = max(v.targetPressure, 0)
        else
          clampedTargetPressure = min(v.targetPressure, relTankPressurePsi)
        end

        local diff = clampedTargetPressure - v.currentPressure
      ...
      local pressureDiff = v.currentPressure - prevPressure
      local energyDiff = pressureDiff * psiToPascal * beamVolume

      if k == "frontDoors" then
        --streams.drawGraph(k .. "_volume", { value = beamVolume })
      end

      --streams.drawGraph(k .. "_dE", { value = energyDiff })

      v.storedEnergy = max(0, v.storedEnergy + energyDiff)

      if airTank and energyDiff > 0 then
        -- air energy must move from the tank into the beam "cylinder" (when moving out of the cylinder, it is released to the atmosphere)
        airTank.storedEnergy = max(0, airTank.storedEnergy - energyDiff)
      end
    end
  ...
  for i = 0, tableSizeC(beams) - 1 do
    ...
    if v.name and pressuredBeamNames[v.name] then
      relevantBeams[v.name] = {
        cid = v.cid,
        defaultPressure = v.pressurePSI,
        maxPressure = v.maxPressurePSI,
        spring = v.beamSpring,
        damp = v.beamDamp,
        surface = v.surface,
      }
  ...
  for _, pressureData in pairs(pressureBeamData) do
    ...
    if not relevantBeams[name] then
      ...
      local beamData = {
        ...
        storedEnergy = (relevantBeams[name].defaultPressure * psiToPascal) * beamVolume,
