  ...
  function postLink(scope, element, attr, ngModelCtrl) {
    ...
    function stepValidator(value) {
      if (angular.isNumber(value)) {
        ...
        // Format to 14 digits after the decimal point - fixes #2015 (...without breaking the use case of a md-slider with min="1.3010299956639813" max="4.342422680822207" -- bruno <bgonzalez@beamng.gmbh>)
        return (Math.round(formattedValue * 1e14) / 1e14);
      }
    }
  ...
  function postLink(scope, element, attr) {
    ...
    function bindEvents () {
      ...
      var enterHandler = function() {
        parent.on('blur focusout mouseleave touchend touchcancel', leaveHandler);
        $rootElement.on("mdtooltiphide", leaveHandler);
        setVisible(true);
      };
      var leaveHandler = function () {
        var autohide = scope.hasOwnProperty('autohide') ? scope.autohide : attr.hasOwnProperty('mdAutohide');
        if (autohide || mouseActive || ($document[0].activeElement !== parent[0]) ) {
          parent.off('blur focusout mouseleave touchend touchcancel', leaveHandler );
          $rootElement.off("mdtooltiphide", leaveHandler);
          setVisible(false);
        }
        mouseActive = false;
      };
      ...
      parent.on('focus focusin mouseenter touchstart', enterHandler );
