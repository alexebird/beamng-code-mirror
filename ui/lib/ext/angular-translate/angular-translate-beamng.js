  ...
  this.$get = ['$log', '$injector', '$rootScope', '$q', function ($log, $injector, $rootScope, $q) {
    ...
    $translate.instant = function (translationId, interpolateParams, interpolationId, forceLanguage, sanitizeStrategy) {
      ...
      if (translationId === null || angular.isUndefined(translationId)) {
        if(MARK_TRANSLATIONS > 1) return "❌ID="+translationId;
        return translationId;
      }
      ...
      if (!result && result !== '') {
        if ($notFoundIndicatorLeft || $notFoundIndicatorRight) {
          ...
          // Once a language is loaded, return the id as valid result. This enables the UI to get
          // stable with unknown keys and is super important to work.
          if (Object.keys($translationTable).length > 0) {
            if(MARK_TRANSLATIONS > 1) return "❌ID="+translationId;
            return translationId;
          }

          // BEAMNG HACK: return undefined until the language is loaded.
          // this will make on-time bindings (::) work properly
          // it will evaluate the {{:: 'foobar' | translate}} filter as 'unstable' and apply the correct settings ONCE the translations are available
          result = undefined
        }
      }
      if(MARK_TRANSLATIONS > 2) return "✅ID="+translationId+" = "+result;
      if(MARK_TRANSLATIONS > 1) return "✅"+result;
      return result;
    };
...
function translateFilterFactory($parse, $translate) {
  ...
  var translateFilter = function (translationId, interpolateParams, interpolation, forceLanguage) {
    if(translationId === undefined) return ''; // fail instantly
    //console.log(translationId, interpolateParams, interpolation, forceLanguage)
