...
.directive('radialmenu', ['mdx', '$http',"$translate", function (mdx, $http,$translate) {
  return {
    template: `
    ...
    link: function (scope, element, attrs) {
      ...
      var accent     = "rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][600].value.toString()+", 1.00)"
      var accentLight= "rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][400].value.toString()+", 0.85)"
      var accentLight2="rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][200].value.toString()+", 0.85)"
      var accentLight3="rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][100].value.toString()+", 0.85)"
      var primary    = "rgba("+mdx.mdxThemeColors._PALETTES['customPrimary'   ][400].value.toString()+", 0.85)"
      var primaryLight="rgba("+mdx.mdxThemeColors._PALETTES['customPrimary'   ][100].value.toString()+", 0.85)"
      var background = 'rgba(0,0,0,0.9)'; // "rgba("+mdx.mdxThemeColors._PALETTES['customBackground'][700].value.toString()+", 0.85)"
      var warn       = "rgba("+mdx.mdxThemeColors._PALETTES['customWarn'      ][400].value.toString()+", 0.85)"
      ...
      var NewAccentLight= "rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][400].value.toString()+", 0.35)"
      var NewAccentLight2LeRetour= "rgba("+mdx.mdxThemeColors._PALETTES['customAccent'    ][400].value.toString()+", 0.7)"
      var rgb = mdx.mdxThemeColors._PALETTES['customAccent'    ][400].value
      ...
      scope.enabled = false
      scope.centerMouseAfterVisible = false
      scope.data = null
      ...
      //console.info('QUICKMENU CREATED')

      //bngApi.engineLua('core_quickAccess and core_quickAccess.isEnabled()', (enabled) => {
      //  scope.enabled = enabled
      //})

      scope.$on('quickAccessEnabled', (ev, _enabled, centerMouse) => {
        scope.enabled = _enabled
        scope.centerMouseAfterVisible = centerMouse
      })

      scope.$on('$destroy', () => {
        //console.info('QUICKMENU DESTROYED')
      })
      ...
      function refreshMenuTitle()
      {
        ...
        switch(scope.data.title.length){
          ...
          case 1:
            menuTxt[0].text($translate.instant(scope.data.title[0], scope.data.context[0]))
            menuTxt[2].text("")
            menuTxt[1].text("")
          break
          case 2:
            menuTxt[1].text($translate.instant(scope.data.title[0], scope.data.context[0]))
            menuTxt[0].text($translate.instant(scope.data.title[1], scope.data.context[1]))
            menuTxt[2].text("")
          break
          default: //3 or more
            var l = scope.data.title.length
            menuTxt[1].text($translate.instant(scope.data.title[l-3], scope.data.context[l-3]))
            menuTxt[0].text($translate.instant(scope.data.title[l-2], scope.data.context[l-2]))
            menuTxt[2].text($translate.instant(scope.data.title[l-1], scope.data.context[l-1]))
          break
        }
      ...
      function redraw() {
        ...
        for(i = 0; i < sliceCount; i++) {
          ...
          if (d.icon  !== undefined) {
            ...
            if(d.icon.indexOf(".svg")!==-1 ){
              ...
              $http({
                    method: 'GET',
                    url: "/ui/modules/apps/RadialMenu/mods_icons/"+d.icon,
                    parentEl: g,
            ...
            /*for(il in IconList){
              if(IconList[il].name.toLowerCase() === d.icon.toLowerCase()){
                if(IconList[il].material === undefined || IconList[il].material === null){
                  ...
                  t.n.innerHTML = IconList[il].material; //fix HU library escaping the & to &amp
        ...
        //description bottom
        var circlePart = (Math.PI / 12)
        var descCircumference = ((2 * Math.PI * (radiusCat1 + (radiusCat2 - radiusCat1) * 0.5)) / 12 * 6) * 0.5; // size of the text description in pixels
        startAngle = circlePart * 3
        endAngle = circlePart * 9
        ...
        itm.push(g)
        var textPath = hu('<textPath>', g)
        textPath.n.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#pathDesc")
        //textPath.n.setAttribute("startOffset","50%")
        //textPath.n.setAttribute("text-anchor","middle")
        itm.push(textPath)

        descTxt = hu('<tspan>', textPath)
        itm.push(descTxt)

        // to track the real width of things and to figure out how much to scroll ...
        descTxtHidden = hu('<text>', svg).css({fill:"white",'font-family':'News Cycle','font-size': fontSizeTXT*0.75, 'letter-spacing':"1px", 'visibility':'hidden' })
        itm.push(descTxtHidden)
        ...
        setDescription = function(txt) {
          ...
          // you need to use a hidden element to determine the size needed. If you warp the text along a path or use the computed lenght, the results are wrong
          descTxtHidden.text(txt)
          var textWidth = descTxtHidden.n.getBBox().width


          var containerWidth = descCircumference; //descTxt.n.parentElement.getBBox().width
          var textOverlap = textWidth - containerWidth
          //console.log(txt, "textWidth = ", textWidth, "containerWidth: ", containerWidth, "textOverlap: ", textOverlap)
          if(textOverlap > 0) {
            textPath.n.setAttribute("startOffset","0")
            textPath.n.setAttribute("text-anchor","start")

            if(descAnim) {
              descAnim.remove()
            }

            descAnim = hu('<animate>', textPath)
            itm.push(descAnim)

            descAnim.n.setAttribute('attributeName', 'startOffset')
            descAnim.n.setAttribute('dur', '10s')
            descAnim.n.setAttribute('calcMode', 'spline')
            descAnim.n.setAttribute('keyTimes', '0 ; 0.5 ; 1')
            descAnim.n.setAttribute('keySplines', '0.42 0 0.58 1 ; 0.42 0 0.58 1'); // easy-in-out
            descAnim.n.setAttribute('repeatCount', 'indefinite')

            var animPadding = 10; // px
            var x = -textOverlap.toFixed() - animPadding
            descAnim.n.setAttribute('values', x + ' ;' + animPadding + ' ;' + x + '')
          } else {
            textPath.n.setAttribute("startOffset","50%")
            textPath.n.setAttribute("text-anchor","middle")
            if(descAnim) {
...
*/
      ...
      let lastMovementWasGamepadAndValid = false
      function updateFocus() {
        ...
        if(scope.controller == 'gamepad') {
          ...
          if(lastMovementWasGamepadAndValid) {
        ...
        if(Math.abs(dx) < radius1noOf * radius1noOf || Math.abs(dx)>(radius2noOf*radius2noOf)) {
          valid = false
          // skip the rest if the gamepad stick moved within the deadzone
          if (scope.controller == 'gamepad' && !lastMovementWasGamepadAndValid) {
            return
          }
        }

        lastMovementWasGamepadAndValid = (scope.controller == 'gamepad') && valid
        ...
        if(!valid) {
          ...
          itemTxt.text("")
          itemSecLineTxt.text("")
          setDescription(''); //'0000 1111 2222 3333 4444 5555 6666 7777 8888 9999 aaaa bbbb cccc dddd eeee ffff'
        ...
        if(focus != oldFocus) {
          ...
          if(focus !== undefined && sliceTexts[focus] !== undefined) {
            ...
            if( sliceTexts[focus].indexOf("ui.radialmenu2.") !== -1){
              format4itemTXT($translate.instant(sliceTexts[focus], sliceContexts[focus]), radius4*1.95)
              setDescription($translate.instant(sliceTexts[focus]+".desc", sliceContexts[focus]))
            }
            else{
              format4itemTXT(sliceTexts[focus], radius4*1.95)
              setDescription(DescTexts[focus])
            }
      ...
      // TODO - When it is ready, use UINav stuff correctly below, instead of hooking event directly (bit of a hack to keep it working for now)

      // direct binding via the input system
      let gamePadLX = 0
      let gamePadLY = 0
      scope.$on('UINavigation', function (event, action, value) {
      // scope.$on('MenuItemNavigation', function (event, action, value) {
        ...
        switch (action) {
          ...
          // case 'radial-x':
             ...
             gamePadLX = Math.max(Math.min( value * Math.sqrt(1 - 0.5* Math.pow(gamePadLY, 2) ) ,1),-1)
             lx = gamePadLX
             ly = gamePadLY
          ...
          // case 'radial-y':
             scope.controller = 'gamepad'
             gamePadLY = Math.max(Math.min( value * Math.sqrt(1 - 0.5* Math.pow(gamePadLX, 2) ) ,1),-1)
             lx = gamePadLX
             ly = gamePadLY
             if(focus !== null && focus !== undefined) bngApi.engineLua('core_quickAccess.moved()')
             break
          case 'back':
            if (value==1) goBack()
            // goBack()
      ...
      scope.$on('QuickAccessMenu', function (event, data) {
        ...
        scope.$applyAsync(function(){
          ...
          if(scope.centerMouseAfterVisible && circle !== null && circle.n !== null) {
            let rect = circle.n.getBoundingClientRect()
            console.log("bounding box:", circle.n.getBoundingClientRect())
            let centerX = rect.left + rect.width * 0.5
            let centerY = rect.top + rect.height * 0.5
            bngApi.engineLua(`extensions.core_quickAccess.centerMouseCallback(${centerX}, ${centerY})`)
          }
        })
      })
