...
// .value('AutomationFilter', {filter_query: '', filter_order_by:'update', filter_order:'desc', currentPage:1, category:[3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 15], subscribed_only: false })
.value('AutomationFilter', {filter_query: '', filter_order_by:'update', filter_order:'desc', currentPage:1, category:[16], subscribed_only: false })
.value('AutomationCategories',[
  // { category: 'vehicles', name: 'ui.repository.cat_veh_land',     value:16  , originalTxt:"Land"},
...
**/
.controller('AutomationController',  ['$scope', '$location', '$state', '$timeout', 'AutomationFilter', '$sce', '$sanitize', 'Utils', 'AutomationCategories',
  function($scope, $location, $state, $timeout, AutomationFilter, $sce, $sanitize, Utils, AutomationCategories) {
  var vm = this
  ...
  vm.categories = AutomationCategories
  vm.category = AutomationFilter.category
  vm.filter_order = AutomationFilter.filter_order
  vm.filter_order_by = AutomationFilter.filter_order_by
  vm.filter_query  = AutomationFilter.filter_query
  vm.currentPage = AutomationFilter.currentPage
  vm.subscribed_only = AutomationFilter.subscribed_only

  $scope.$on("$destroy", function() {
    AutomationFilter.category = vm.category
    AutomationFilter.filter_order = vm.filter_order
    AutomationFilter.filter_order_by = vm.filter_order_by
    AutomationFilter.filter_query = vm.filter_query
    AutomationFilter.currentPage = vm.currentPage
    AutomationFilter.subscribed_only = vm.subscribed_only
  ...
  $scope.showMods = function (event, pnewdata) {
    vm.loadingPage = false
    vm.loadTimeout = false
    if (pnewdata.automationMsg) {
      vm.automationMsg = pnewdata.automationMsg
      vm.automationMsg.msg = $sce.trustAsHtml(Utils.parseBBCode($sanitize(pnewdata.automationMsg.msg)))
    }
  ...
  vm.select = function (mod, id, subscribe) {
    $state.go('menu.mods.automationDetails', {modId: mod.tagid})
    vm.selected = mod
    vm.selectedId = id
...
.controller('AutomationDetailsController',  ['$location', '$scope', '$stateParams', 'Utils','$sce','$sanitize', '$timeout', '$http','$state','AutomationFilter','AutomationCategories','$filter',
  function($location, $scope, $stateParams,Utils,$sce,$sanitize,$timeout,$http,$state,AutomationFilter,AutomationCategories,$filter) {
  var vm = this
  ...
  $scope.openAuthor = function(name){
    AutomationFilter.filter_query = name
    AutomationFilter.currentPage = 1
    AutomationFilter.filter_order_by='downloads'
    AutomationFilter.filter_order='desc'
    AutomationFilter.category=[16]
    $state.go('menu.mods.automation')
  }

  // $scope.openCat = function(){
  //   AutomationFilter.filter_query = ""
  //   AutomationFilter.currentPage = 1
  //   AutomationFilter.filter_order_by='downloads'
  //   AutomationFilter.filter_order='desc'
