...
<script setup>
import { ref } from "vue"
import { BngList, BngListItemPart, BngSwitch } from "@/common/components/base"
...
for (let i = 1; i <= 8; i++) {
  sampleData1.push({
    label: "Object item " + i,
    icon: iconTypesList[Math.floor(Math.random() * iconTypesList.length)],
  })
}
sampleData1.push({
  label: "Disabled item",
