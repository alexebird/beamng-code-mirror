    ...
    <div v-if="players.length > 1 || (bindings[show] && bindings[show].length > 0)" class="bindings-app">
        <div @click="toggleSmall()" class="binding-show">
            <span v-if="small" class="toggle-icon">
                <BngIcon class="key-icon" :type="icons.general.arrow_small_left" />
            </span>
            <span v-else class="toggle-icon">
                <BngIcon class="key-icon" :type="icons.general.arrow_small_right" />
            </span>
        </div>
        <div class="players-binding" v-if="!small && (players.length > 1 || (bindings[show] && bindings[show].length > 0))">
            <div v-if="!small && players.length > 1">
                <BngButton v-if="bindings.length > 1" @click="backward()"></BngButton>
                <span>Player {{ show }}</span>
                <BngButton v-if="bindings.length > 1" @click="forward()"></BngButton>
            </div>
            <div v-if="bindings[show].length > 0 && !small" class="bindings-container">
                <div v-for="entry in bindings[show]" class="binding-item">
                    ...
                    <div>
                        <BngBinding v-for="b in entry.control" :deviceKey="b.c" :device="b.d" :show-unassigned="true"
                            @click="goToBindings(entry, b)"></BngBinding>
                    </div>
                </div>
            </div>
        </div>
...
import { BngButton, BngBinding, BngIcon } from '@/common/components/base'
