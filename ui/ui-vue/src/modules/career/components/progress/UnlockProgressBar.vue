<template>
  <div class="bng-progress-bar">
    <div v-if="headerLeft || headerRight" class="header">
      <span class="header-left"> {{ headerLeft }} Progress: Level {{ currentTier }} </span>
    </div>
    <div class="progress-bar">
      <div class="tiers-bar" v-if="tiers" :style="{ '--num-columns': tiers.length }">
        <template class="tier" v-for="tier in tiers">
          <div :class="{ 'tier-label': true, 'grayed-out': currentTier < tier.index }" :style="{ gridColumn: tier.index }">Level {{ tier.index }}</div>
          <div :class="{ 'tier-info': true, 'grayed-out': currentTier < tier.index }" :style="{ gridColumn: tier.index }">
            <UnlockCard class="tier-unlocks-item" :data="formatTier(tier)" />
          </div>
          <div :class="{ 'tier-unlocks': true, 'grayed-out': currentTier < tier.index }" :style="{ gridColumn: tier.index }">
            <div class="tier-unlocks-label">Unlocks</div>
            <UnlockCard v-for="item in tier.list" class="tier-unlocks-item" :data="item" />
          </div>
          <div :class="{ 'tier-progress': true, unlockable: currentTier < tier.index && !tier.isInDevelopment }" :style="{ gridColumn: tier.index }">
            <div v-if="tier.isBase" class="tier-value"></div>
            <div v-else-if="tier.isInDevelopment" class="tier-value">
              <BngIcon class="tier-header-block-icon" :type="icons.roadblockL" /><span>Coming Soon!</span>
            </div>
            <div v-else-if="currentTier + 1 < tier.index" class="tier-value">
              <BngIcon class="tier-header-block-icon" :type="icons.lockClosed" /><span>Locked</span>
            </div>
            ...
            <div v-else class="tier-value">
              <BngIcon class="tier-header-block-icon" :type="icons.lockClosed" /><span>{{ tier.currentValue }} / {{ tier.requiredValue }} XP</span>
            </div>
          ...
          <div
            :class="{
              'main-track': true,
              'progress-fill-full-bg': currentTier + 1 > tier.index,
              current: currentTier + 1 == tier.index,
              indev: tier.isInDevelopment,
            }"
  ...
  .tiers-bar {
    ...
    .tier-progress {
      ...
      .progressbar-fill {
        ...
        bottom: 0;
        left: 0;
        background: linear-gradient(to top, rgba($progress-fill-color, 1), rgba($progress-fill-color, 1) 2.5rem, rgba(black, 0) 2.501rem),
          border-box 50% 100% / 100% 105%
            radial-gradient(circle at center bottom, rgba($progress-fill-color, 0.6), rgba($progress-fill-color, 0.4) 25%, rgba($progress-fill-color, 0) 80%);
        z-index: -1;
      }
    }
    .main-track {
      ...
      &.indev {
        background: repeating-linear-gradient(-45deg, rgba(white, 0.1), rgba(white, 0.1) 1rem, rgba(white, 0) 1rem, rgba(white, 0) 2rem);
      }
    }
  }
}
